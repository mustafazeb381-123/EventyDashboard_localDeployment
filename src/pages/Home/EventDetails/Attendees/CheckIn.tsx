...
function CheckIn() {
  ...
  const fetchUsers = async (id: string, areaId: string | number) => {
    ...
    try {
      // Use getCheckIns the same way as GateOnboarding.tsx
      const response = await getCheckIns(id, areaId);
  ...
  const handleExportCsv = () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `check_in_need_check_in_${eventId}_${sessionAreaId}_${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `check_in_need_check_in_${eventId}_${sessionAreaId}_${new Date().toISOString().slice(0, 10)}.xlsx`;
      a.click();
      URL.revokeObjectURL(url);
