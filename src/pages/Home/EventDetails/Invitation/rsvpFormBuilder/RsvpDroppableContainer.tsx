...
interface RsvpDroppableContainerProps {
  field: RsvpFormField;
  childFields: RsvpFormField[];
  allFields: RsvpFormField[];
  onEdit: (field: RsvpFormField) => void;
  onDelete: (id: string) => void;
  onEditChild: (field: RsvpFormField) => void;
  onDeleteChild: (id: string) => void;
  onUpdate?: (field: RsvpFormField) => void;
  onUpdateChild?: (field: RsvpFormField) => void;
}

export const RsvpDroppableContainer: React.FC<RsvpDroppableContainerProps> = ({
  field,
  childFields,
  allFields,
  onEdit,
  ...
  return (
    <div
      ...
      } ${isOver ? "ring-2 ring-indigo-400 bg-indigo-100" : ""}`}
      {...attributes}
      {...listeners}
    >
      <div className="absolute -top-3 left-4 bg-white px-2">
        {isEditingLabel ? (
          <input
            ...
            className="text-xs font-semibold text-gray-700 uppercase tracking-wider bg-white border border-indigo-500 rounded px-2 py-0.5 outline-none focus:ring-2 focus:ring-indigo-300 min-w-[100px]"
            autoFocus
          ...
          <span
            className="text-xs font-semibold text-gray-700 uppercase tracking-wider cursor-text hover:text-indigo-600 hover:underline"
            onClick={(e) => {
              e.stopPropagation();
      ...
      <div
        ...
        className={`min-h-[60px] space-y-2 ${
          isEmpty ? "border-2 border-dashed border-slate-300 rounded-lg p-4" : ""
        }`}
      >
        <SortableContext
              ...
              if (childField.containerType) {
                const nestedChildFields = childField.children
                  ? (childField.children
                      ...
                      .filter(Boolean) as RsvpFormField[])
                  : [];

                return (
                  <RsvpDroppableContainer
                    key={childField.id}
                    field={childField}
