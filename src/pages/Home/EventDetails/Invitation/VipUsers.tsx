...
function VipUsers() {
  ...
  const fetchUsers = async (id: string, page: number = 1) => {
    ...
    try {
      ...
      // Filter for VIP users only (user_type is "VIP" or "vip")
      const vipUsers = allUsers.filter((user: any) => {
      ...
      if (paginationMeta) {
        ...
        setPagination({
          ...paginationMeta,
          total_count: vipUsers.length,
          total_pages: Math.ceil(vipUsers.length / itemsPerPage),
        });
      }
    } catch (error) {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      // Always set user_type to VIP for VIP users
      formData.append("event_user[user_type]", "VIP");
      ...
      // Debug: Log form data
      console.log("📤 Creating VIP user with:", {
      ...
      showNotification("VIP User created successfully!", "success");
                  ...
                  {loadingUsers ? (
                    ...
                    <tr>
                      <td colSpan={8} className="px-6 py-8 text-center text-gray-500">
                        {searchTerm ? `No VIP users found matching "${searchTerm}"` : "No VIP users found"}
                      </td>
                    </tr>
                  ) : (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-gray-200/60 sticky top-0 bg-white z-10">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Add New VIP User</h2>
                <p className="text-gray-600 mt-1">Create a new VIP user for this event</p>
              </div>
              <button
                onClick={() => !isCreatingUser && setShowAddUserModal(false)}
