...
const TemplateSix = ({
  ...
  console.log(
    "onUse template TemplateSix-----+++++------",
    onUseTemplate,
    eventId,
    data
  );

  console.log("form data in template six ::::", data);

  // Handle the use template click
  const handleUseTemplate = () => {
    console.log("TemplateSix handleUseTemplate");
    if (onUseTemplate && !isLoading) {
      // Call the parent function with template data
      onUseTemplate("template-six", {
        name: "Event Registration Form Template Six",
        description:
          "An innovative registration form template with advanced features for tech conferences.",
