// Updated TemplateForm.jsx with dynamic functionality and banner upload
...
function TemplateFormSix({
  ...
  useEffect(() => {
    ...
    const fetchApiFormData = async () => {
      ...
      try {
        ...
        console.log(
          "TemplateSix - getRegistrationFieldApi response:",
          response.data
        );
        setApiFormData(response.data.data || []);
      } catch (error) {
        console.error("TemplateSix - Failed to get registration field:", error);
        setApiFormData([]);
      } finally {
  ...
  const fetchEventData = async () => {
    ...
    try {
      const response = await getEventbyId(effectiveEventId);
      console.log("Event data fetched in TemplateSix :: ", response.data.data);
