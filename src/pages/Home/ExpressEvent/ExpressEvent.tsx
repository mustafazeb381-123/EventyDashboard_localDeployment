...
const ExpressEvent = () => {
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar">
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                <div className={`flex items-center gap-2 font-medium text-xs font-poppins ${textColor}`}>
                  {isCompleted && <CheckCircle className={`w-4 h-4 ${iconColor}`} />}
                  {step.label}
                </div>
                <div className={`text-xs font-normal font-poppins ${descriptionColor} leading-tight mt-1`}>
                  {step.description}
                </div>
              </div>
            );
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 1:
         return <RegistrationForm onNext={handleNext} onPrevious={handlePrevious} currentStep={currentStep} totalSteps={steps.length} />
      case 2:
        return (
