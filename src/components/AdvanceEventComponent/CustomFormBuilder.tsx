  ...
  const handleAddField = (field: CustomFormField) => {
    setFields((prev) => {
      ...
      const looksAutoGenerated =
        ...
        field.name.startsWith(`${field.type}_`);

      const desiredName = looksAutoGenerated
        ? makeFieldNameFromLabel(label)
        : field.name;

      const uniqueName = ensureUniqueFieldName(desiredName, field.id, prev);

      const desiredPlaceholder =
